...
def editar_paciente(id):
    # Busca al paciente por su ID, si no existe manda un error 404 controlado
    paciente = Paciente.query.get_or_404(id)

    if request.method == 'POST':
        # Mapea los datos del formulario HTML con los campos de tu modelo Paciente
        paciente.nombre = request.form['nombre']
        paciente.edad = request.form['edad']
        paciente.direccion = request.form['direccion']
        paciente.telefono = request.form['telefono']

        # Guarda los cambios en la base de datos de SQLite
        db.session.commit()

        # Te redirige de vuelta a la lista general de pacientes
        return redirect(url_for('main.pacientes'))

    # Si entra por GET, renderiza la plantilla para editar pasando los datos del paciente
    return render_template(
